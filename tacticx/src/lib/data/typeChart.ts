...
/**
 * Gen 9 type effectiveness chart. TYPE_CHART[attackType][defenseType] = multiplier.
 * Source: https://bulbapedia.bulbagarden.net/wiki/Type
 * Only non-neutral matchups are listed; everything else defaults to 1.
 */
...
export const TYPE_CHART: Record<PokemonType, Record<PokemonType, number>> = (() => {
  ...
  for (const atk of TYPE_ORDER) {
    ...
    for (const def of TYPE_ORDER) {
      chart[atk][def] = OVERRIDES[atk]?.[def] ?? 1
    }
  }
  return chart
})()
...
/** Single-type lookup. */
export function effectiveness(attackType: PokemonType, defenseType: PokemonType): number {
